...
parseInt = parse intParser
   . substitute [(",",""), ("illions","illion"), ("thousands","thousand")]
   . map toLower
   where
...
parseLarge = parse ((match "thousand" ~> const 0) <> largeParser) . map toLower
   where
